...
const QUICK_SELECT_PREFIX_APP = 'app';
const QUICK_SELECT_PREFIX_FILE = 'file';
...
class CinnamenuApplet extends TextIconApplet {
    constructor(metadata, orientation, panel_height, instance_id) {
        ...
        [
        { key: 'quick-select-items',       value: 'quickSelectItems',      cb: null },
        { key: 'quick-select-migrated',    value: 'quickSelectMigrated',   cb: null },
...
//==================================================================
    ...
    _getQuickSelectItems() {
        if (!Array.isArray(this.settings.quickSelectItems)) {
            this.settings.quickSelectItems = [];
        }
        return this.settings.quickSelectItems;
    }
    ...
    _getQuickSelectKeyForItem(item) {
        ...
        if (item.isApplication && item.id) {
            return this._makeQuickSelectEntry(QUICK_SELECT_PREFIX_APP, item.id);
        }
        if ((item.isFavoriteFile || item.isFolderviewFile || item.isRecentFile) && item.uri) {
            return this._makeQuickSelectEntry(QUICK_SELECT_PREFIX_FILE, item.uri);
        }
        return null;
    }
    ...
    _seedQuickSelectItems() {
        ...
        const addUnique = (value) => {
            if (value && seededItems.indexOf(value) < 0) {
                seededItems.push(value);
            }
        };

        if (this.settings.sidebarFavorites === 1 || this.settings.sidebarFavorites === 3) {
            this.appFavorites.getFavorites().forEach(favApp => {
                addUnique(this._makeQuickSelectEntry(QUICK_SELECT_PREFIX_APP, favApp.id));
            });
        }
        if (this.settings.sidebarFavorites === 2 || this.settings.sidebarFavorites === 3) {
            XApp.Favorites.get_default().get_favorites(null).forEach(info => {
                addUnique(this._makeQuickSelectEntry(QUICK_SELECT_PREFIX_FILE, info.uri));
            });
        }

        if (seededItems.length > 0) {
            this.settings.quickSelectItems = seededItems;
        }
    }
    ...
    _ensureQuickSelectItems() {
        if (!this.settings.quickSelectMigrated) {
            ...
            this.settings.quickSelectMigrated = true;
        }
    }
    ...
    addQuickSelectItemToPos(item, posItem) {
        ...
        const posKey = this._getQuickSelectKeyForItem(posItem);
        if (posKey) {
            const posIndex = items.indexOf(posKey);
            if (posIndex >= 0) {
                insertAt = posIndex;
            }
        }
        items.splice(insertAt, 0, itemKey);
        this._setQuickSelectItems(items);
    }
    ...
    addQuickSelectAppToPos(addId, posId) {
        ...
        const items = this._getQuickSelectItems().slice().filter(entry => entry !== itemKey);
        let insertAt = items.length;
        const posKey = this._makeQuickSelectEntry(QUICK_SELECT_PREFIX_APP, posId);
        const posIndex = items.indexOf(posKey);
        if (posIndex >= 0) {
            insertAt = posIndex;
        }
    ...
    setActiveCategory(categoryId) {
        ...
        // (must be either 'emoji:' (no particular emoji category open) or begin with 'emoji:')
 ...
 */
    ...
    listQuickSelectItems() {
        ...
        this._getQuickSelectItems().forEach(entry => {
            ...
            if (parsed.type === QUICK_SELECT_PREFIX_APP) {
                const app = this.apps.listApplications('all').find(app => app.id === parsed.value);
                if (app) {
                    app.name = app.get_name();
                    app.description = app.get_description();
                    app.isApplication = true;
                    app.isQuickSelectItem = true;
                    res.push(app);
                }
            } else if (parsed.type === QUICK_SELECT_PREFIX_FILE) {
                const file = Gio.File.new_for_uri(parsed.value);
                const fileName = file.get_basename() || parsed.value;
                let fileInfo = null;
                try {
                ...
                const mimeType = fileInfo ? fileInfo.get_content_type() : null;
                const gicon = mimeType ? Gio.content_type_get_icon(mimeType) :
                                            new Gio.ThemedIcon({name: 'text-x-generic'});
                res.push({
                    name: fileInfo ? fileInfo.get_display_name() : fileName,
                    description: file.get_path() || parsed.value,
                    gicon: gicon,
                    isFavoriteFile: true,
                    isQuickSelectItem: true,
    ...
    #initAppCategories() {
        ...
        while ((nextType = iter.next()) !== CMenu.TreeItemType.INVALID) {
            if (nextType === CMenu.TreeItemType.DIRECTORY) {
                ...
                const foundApps = this.#loadDirectory(dir);
        ...
        Object.keys(this.#appsByCategory).forEach(key => {
                    ...
                    if (['Preferences','Administration'].includes(key)) {
                        allSettings = allSettings.concat(this.#appsByCategory[key]);
                    } else {
                        allApps = allApps.concat(this.#appsByCategory[key]);
                    }
        });
        this.#appsByCategory.all = Array.from(new Set(all));//remove duplicates
        apps_sort(this.#appsByCategory.all);
        this.#appsByCategory.allApps = Array.from(new Set(allApps));//remove duplicates
        apps_sort(this.#appsByCategory.allApps);
        this.#appsByCategory.allSettings = Array.from(new Set(allSettings));//remove duplicates
        apps_sort(this.#appsByCategory.allSettings);
        //remove apps from allSettings if they're also in allApps
        this.#appsByCategory.allApps.forEach( app => {
            const i = this.#appsByCategory.allSettings.findIndex( a => a === app );
            if (i > -1) {
                this.#appsByCategory.allSettings.splice(i, 1);
            }
        })
        ...
        while ((nextType = iter.next()) !== CMenu.TreeItemType.INVALID) {
            if (nextType === CMenu.TreeItemType.ENTRY) {
                ...
                if (!subDir.get_is_nodisplay()) {
                    foundApps = foundApps.concat(this.#loadDirectory(subDir));
                }
            }
        }
        return foundApps;
