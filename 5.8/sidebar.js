            ...
            this.actor._delegate = {
                    ...
                    acceptDrop: (source) => {
                            if (source.isDraggableApp === true && source.id !== this.app.id) {
                                ...
                                this.appThis.addQuickSelectAppToPos(source.id, this.app.id);
                                return true;
                            } else {
                                this.actor.set_opacity(255);
                                return DragMotionResult.NO_DROP;
...
class Sidebar {
    ...
    populate () {
        ...
        //----add quick select items to this.items[]
        this.appThis.listQuickSelectItems().forEach(item => {
            if (this.separator1Position === null) {
                this.separator1Position = this.items.length;
            }
            if (item.isApplication) {
                this.items.push(new SidebarButton( this.appThis,
                                item.create_icon_texture(this.appThis.settings.sidebarIconSize),
                                        item, item.name, item.description, null));
            } else if (item.isFavoriteFile) {
                let gicon = getThumbnail_gicon(item.uri, item.mimeType) || item.gicon;
                this.items.push(new SidebarButton( this.appThis,
